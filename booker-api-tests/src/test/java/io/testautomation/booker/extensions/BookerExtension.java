package io.testautomation.booker.extensions;
...
public final class BookerExtension implements BeforeEachCallback, AfterEachCallback, ParameterResolver {
    ...
    private static String featureName(Class<?> testClass) {
        if (testClass.isAnnotationPresent(AuthenticationFeature.class)) {
            return "Authentication";
        }
        if (testClass.isAnnotationPresent(BookingFeature.class)) {
            return "Booking";
        }
        throw new IllegalStateException("Booker test class must declare a supported feature annotation: "
                + testClass.getName());
    }
